    ...
    >
      ...
      <CardBody className="p-5 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center text-sm mt-1">
          ...
          <span className="!text-black font-black dark:!text-gray-100 font-mono">
            {formattedDate}
          </span>
        </div>
        <div className="pt-2 grid grid-cols-2 gap-3">
          <Button
            as={Link}
            to={`/tickets/${reg.uuid}`}
            className="w-full h-10 font-semibold rounded-lg flex items-center justify-center gap-2 border border-[#03a1b0] text-[#03a1b0] bg-transparent hover:bg-[#03a1b0]/10 dark:border-cyan-500 dark:text-cyan-400 transition-colors"
          >
            <Eye size={18} /> Details
          </Button>
          <Button
            as={Link}
            to={`/tickets/${reg.uuid}`}
            className="w-full h-10 font-semibold rounded-lg text-white flex items-center justify-center gap-2 bg-[#03a1b0] hover:bg-[#008b99] shadow-md transition-all"
          >
    ...
    <section className="relative w-full min-h-screen mx-auto px-0 sm:px-16 space-y-8">
      ...
      {filteredData.length !== TOTAL && (
        <div className="flex items-center gap-2 text-sm text-gray-500 -mt-4 px-1">
          <span>
            Found <b>{filteredData.length}</b> results
          </span>
          ...
          slotProps={{
            select: {
              MenuProps: {
                PaperProps: {
                  className:
                    "!bg-inherit !bg-transparent !text-gray-900 dark:!bg-inherit dark:!text-gray-100 !backdrop-blur-md",
                  sx: {
