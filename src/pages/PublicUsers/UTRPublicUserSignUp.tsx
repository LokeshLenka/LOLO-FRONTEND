...
export const UtrPublicUserSignUp: React.FC = () => {
    ...
    const formSchema = useMemo(() => {
        return z.object({
            ...
            // If fee > 0, enforce utr_number length to be at least 8.
            if (currentFee > 0) {
                if (!data.utr_number || data.utr_number.trim().length < 12) {
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ["utr_number"],
                        message: "Valid UTR Number is required (at least 12 digits) for paid events.",
                    });
                }
            }
        });
    ...
    const handleRegistration = async (data: FormData) => {
        ...
        try {
            ...
            const regRes = await api.post(`/public/event/${eventId}/registration`, payload);
            ...
            toast.success(isUtrPayment ? "Registration submitted successfully! Awaiting UTR verification." : "Registration Successful!", { id: toastId });
            ...
            <div className="relative z-10 w-full max-w-6xl px-4 pt-24 pb-32">
                <div className="text-center mb-12">
                    ...
                    <p className="text-neutral-400 text-lg max-w-2xl mx-auto">
                        {eventName ? `Registering for ${eventName}` : "Enter your details to register."}
                    </p>
                </div>
                                    ...
                                    <div className="bg-[#09090b]/80 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                                        ...
                                        {currentFee > 0 && (
                                            <>
                                                ...
                                                <div className="mb-6 flex flex-col items-center justify-center p-6 bg-white/5 border border-white/10 rounded-2xl relative z-10">
                                                    ...
                                                    <p className="text-neutral-400 text-xs text-center">Please pay using UPI before proceeding</p>
                                                </div>
                                        ...
                                        <div className="space-y-4 mb-8 relative z-10 border-t border-white/10 pt-6">
                                            <div className="flex justify-between items-center text-lg font-bold">
                                                ...
                                                <span className="text-3xl text-transparent bg-clip-text bg-gradient-to-r from-lolo-pink to-purple-400 font-mono tracking-tight">
                                                    {isFeeLoaded ? (currentFee === 0 ? "Free" : `₹${currentFee}`) : <Loader2 className="w-6 h-6 animate-spin text-lolo-pink" />}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        {!isFeeLoaded && (
                                            <div className="flex items-center justify-center gap-2 p-3 mb-6 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-200 text-xs relative z-10">
                                                <Loader2 className="w-3 h-3 animate-spin" /> Fetching latest ticket prices...
                                            </div>
                                        )}
